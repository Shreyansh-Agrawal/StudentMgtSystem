...
def add_registration(roll_no: int, course_id: str, date_of_entry: str) -> None:
    with DatabaseConnection('data.db') as connection:
        ...
        try:
            cursor.execute(RegistrationQueries.ENABLE_FOREIGN_KEYS)
            cursor.execute(RegistrationQueries.ADD_REGISTRATION, (roll_no, course_id, date_of_entry))
...
def get_registration(roll_no: int) -> List[Registration]:
    ...
    with DatabaseConnection('data.db') as connection:
        ...
        try:
            ...
            registration = [{'roll_no': row[0], 'course_id': row[1], 'date_of_entry': row[2]} for row in cursor.fetchall()]
        except:
...
def get_all_registrations() -> List[Registration]:
    ...
    with DatabaseConnection('data.db') as connection:
        ...
        try:
            ...
            registrations = [{'roll_no': row[0], 'name': row[1], 'course_id': row[2], 'course_name': row[3],'date_of_registration': row[4]} for row in cursor.fetchall()]
...
def update_registration(roll_no: int, course_id: str, new_course_id: str) -> None:
    with DatabaseConnection('data.db') as connection:
        cursor = connection.cursor()
        try:
            cursor.execute(RegistrationQueries.UPDATE_REGISTRATION, (new_course_id, roll_no, course_id))
            print(f"Updated Course: {new_course_id} from {course_id} for Roll No: {roll_no}")
        except sqlite3.IntegrityError as e:
...
def delete_registration(roll_no: int, course_id: str) -> None:
    with DatabaseConnection('data.db') as connection:
        cursor = connection.cursor()
        try:
            cursor.execute(RegistrationQueries.DELETE_REGISTRATION, (roll_no, course_id))
